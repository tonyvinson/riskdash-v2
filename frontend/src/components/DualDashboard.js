...
// KSI Family definitions (same as RealKSIManagement)
const KSI_FAMILIES = {
...
// Extract family from KSI ID (same as RealKSIManagement)
...
📋 CLI commands will show specific ${category} AWS API calls.`);
  ...
  useEffect(() => {
    console.log('🚀 Enhanced DualDashboard mounted, loading real KSI data...');
    loadEnhancedData();
  }, []);
  ...
  const loadEnhancedData = async () => {
    try {
      ...
      const enhancedKSIs = allKSIs.map(ksiObj => ({
        ksi_id: ksiObj.ksi_id,
        family: extractFamily(ksiObj.ksi_id),
        familyInfo: KSI_FAMILIES[extractFamily(ksiObj.ksi_id)] || { 
          name: 'Unknown', 
          color: 'bg-gray-50 border-gray-200', 
          icon: '❓' 
        },
        title: ksiObj.title || `${extractFamily(ksiObj.ksi_id)} Rule`,
        category: ksiObj.category || KSI_FAMILIES[extractFamily(ksiObj.ksi_id)]?.name || 'Unknown',
        description: ksiObj.description || `Validation rule for ${ksiObj.ksi_id}`,
        automation_type: ksiObj.automation_type || 'manual',
        status: ksiObj.status || 'active',
        validation_steps: ksiObj.validation_steps || 0,
        isEnabled: enabledKSIs.includes(ksiObj.ksi_id)
      }));

      // 🔧 CLEAN CATEGORIZATION: Use real automation_type from API
      const automatedKSIs = enhancedKSIs.filter(ksi => 
        ksi.isEnabled && (
          ksi.automation_type === 'fully_automated' || 
          ksi.automation_type === 'partially_automated' ||
          ksi.validation_steps > 0
        )
      );
      
      const manualKSIs = enhancedKSIs.filter(ksi => 
        ksi.isEnabled && ksi.automation_type === 'manual' && ksi.validation_steps === 0
      );

      console.log(`✅ Real categorization: ${automatedKSIs.length} automated, ${manualKSIs.length} manual (based on actual automation_type from API)`);

      // 🔍 Deduplicate results
      const results = getLatestResultsPerKSI(rawResults);
      
      // Calculate stats based on enabled KSIs only
      const enabledResults = results.filter(result => enabledKSIs.includes(result.ksi_id));
      const passedKSIs = enabledResults.filter(r => r.assertion === true || r.assertion === "true").length;
      const failedKSIs = enabledResults.filter(r => r.assertion === false || r.assertion === "false").length;
      const pendingKSIs = Math.max(0, enabledKSIs.length - enabledResults.length);

      const compliance = enabledResults.length > 0 ? 
        Math.min(100, Math.round((passedKSIs / enabledResults.length) * 100)) : 0;

      const lastRun = enabledResults.length > 0 ? 
        enabledResults.reduce((latest, current) => 
      ...
      const status = failedKSIs > 0 ? 'critical' : (pendingKSIs > 0 ? 'warning' : 'healthy');

      const priorityItems = enabledResults
        ...
        .concat(
          enabledKSIs
            .filter(ksi => !enabledResults.find(r => r.ksi_id === ksi))
            .map(ksi => ({
              ksi_id: ksi,
      ...
      const finalData = {
        ...
        lastRun,
        compliance,
        issuesCount: failedKSIs + pendingKSIs,
        totalKSIs: enabledKSIs.length,
        passedKSIs,
        failedKSIs,
        pendingKSIs,
        activeKSIs: automatedKSIs.length,
        manualKSIs: manualKSIs.length,
        disabledKSIs: allKSIs.length - enabledKSIs.length,
        executionHistory: enhancedExecutions,
        lastExecutionDetails,
        automatedCompliance: compliance,
        overallCompliance: results.length > 0 ? 
          Math.round((results.filter(r => r.assertion === true || r.assertion === "true").length / results.length) * 100) : 0,
        overallTotalKSIs: allKSIs.length,
        overallPassedKSIs: results.filter(r => r.assertion === true || r.assertion === "true").length,
        overallFailedKSIs: results.filter(r => r.assertion === false || r.assertion === "false").length,
        priorityItems,
        // 🔧 ENHANCED: Real KSI data for reference
        realKSIData: {
          total: allKSIs.length,
          enabled: enabledKSIs.length,
          automated: automatedKSIs.length,
          manual: manualKSIs.length,
      ...
      console.log('✅ Enhanced DualDashboard data compiled successfully:', {
      ...
      console.error('❌ Error loading enhanced DualDashboard data:', error);
      setDashboardData({
        status: 'critical',
        lastRun: null,
        compliance: 0,
      ...
      setLoading(false);
      console.log('🏁 DualDashboard loading complete');
    }
  };
  ...
  const handleKSIManagementSave = (preferences) => {
    ...
    // Note: This would now integrate with the real tenant config API
    loadEnhancedData();
  ...
  if (loading) {
    console.log('⏳ DualDashboard is loading...');
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
        <span className="ml-2">Loading DualDashboard...</span>
      </div>
    );
  }

  console.log('🎨 Rendering DualDashboard with real KSI data:', {
    status: dashboardData.status,
    executionHistory: dashboardData.executionHistory.length,
    selectedView,
    realData: dashboardData.realKSIData
  });
    ...
    <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button 
                onClick={() => {
                  console.log('🔄 Force refreshing DualDashboard data...');
                  loadEnhancedData();
                }}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-white rounded-lg p-6 text-center border shadow-sm">
          ...
          <div className="text-xs text-gray-500 mt-1">Enabled KSIs</div>
        </div>
        <div className="bg-white rounded-lg p-6 text-center border shadow-sm">
          <div className="text-lg font-bold text-green-600">{dashboardData.activeKSIs}</div>
          <div className="text-lg font-bold text-gray-400">{dashboardData.manualKSIs}</div>
                ...
                {dashboardData.status === 'healthy' ? (
                  <div className="space-y-4">
                    ...
                    <p className="text-green-600">
                      {dashboardData.passedKSIs} of {dashboardData.totalKSIs} enabled KSIs are compliant
                    </p>
                  </div>
                ) : dashboardData.status === 'warning' ? (
                  <div className="space-y-4">
                    ...
                    <p className="text-yellow-600">
                      {dashboardData.issuesCount} issues found across {dashboardData.totalKSIs} enabled KSIs
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Enabled KSIs:</span>
                      <span className="font-medium">{dashboardData.totalKSIs}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Automated KSIs:</span>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div>Total KSIs in System: {dashboardData.overallTotalKSIs}</div>
                    <div>Recent Results: {dashboardData.realKSIData?.recentResults || 0}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {dashboardData.priorityItems.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">All enabled KSIs are passing their validations.</p>
                </div>
              ) : (
                <>
